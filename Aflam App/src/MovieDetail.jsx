...
const MovieDetail = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/movies")
      ...
      .then((data) => {
        ...
        const primary =
          matchedGroup.find((m) => m._id === movie_id) || matchedGroup[0];
  ...
  const WIDE_ASPECT_RATIO_THRESHOLD = 2.0; // Width > 2 * height
  const isWideImage =
    imageAspectRatio !== null && imageAspectRatio > WIDE_ASPECT_RATIO_THRESHOLD;
  const imageWidthClass = isWideImage
    ? "md:w-2/3 lg:w-1/2 xl:w-3/5"
    : "md:w-1/3 lg:w-1/4";
  ...
  if (!mainMovie) {
    return (
      <div className="flex flex-col items-center justify-center h-screen text-white bg-gray-900 px-4">
        ...
        <p className="text-red-400 font-bold text-xl mb-4 text-center">
          Oops! Movie Not Found
        </p>
        <p className="text-gray-400 text-center mb-6">
          We couldn't find the details for this movie. It might have been
        ...
        <div className="mt-8 flex flex-col md:flex-row items-start md:gap-12">
          ...
          <div className="flex-grow">
            <p className="text-lg text-gray-300 mb-4">
              {mainMovie.Description || "No description available."}
            </p>
            <div className="space-y-2 text-gray-400 mb-6">
              <p>
                <strong>Classification:</strong>{" "}
                <span className="text-gray-200">{mainMovie.Rating}</span>
              </p>
              <p>
                <strong>Language:</strong>{" "}
                <span className="text-gray-200">{mainMovie.Language}</span>
              </p>
              <p>
                <strong>Genre:</strong>{" "}
          ...
          <div className="mt-10">
            ...
            <div className="flex overflow-x-auto space-x-4 py-2 scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800">
              {cinemas.map((cinema) => (
                <button
                  key={cinema}
                  className={`px-5 py-2.5 rounded-md text-sm font-semibold whitespace-nowrap transition duration-200 ease-in-out border ${
                      ...
                      <div key={exp.Experience}>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {exp.Times.map((time) => {
                            ...
                            if (parent === "amc") {
                              fullUrl =
                                "https://www.amccinemas.com" + showtimeUrl;
                            } else if (parent === "vox") {
                              fullUrl =
                                "https://ksa.voxcinemas.com" + showtimeUrl;
                            } else if (parent === "muvi") {
                              fullUrl =
                                "https://www.muvicinemas.com" + showtimeUrl;
                            }
                            return (
                              <a
                                key={time}
